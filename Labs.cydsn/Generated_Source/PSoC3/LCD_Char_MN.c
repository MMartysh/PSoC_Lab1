...
static void LCD_Char_MN_WrDatNib(uint8 nibble) ;
static void LCD_Char_MN_WrCntrlNib(uint8 nibble) ;

/* Stores state of component. Indicates whether component is or not
* in enable state.
*/
uint8 LCD_Char_MN_enableState = 0u;

uint8 LCD_Char_MN_initVar = 0u;


/*******************************************************************************
* Function Name: LCD_Char_MN_Init
********************************************************************************
*
* Summary:
...
void LCD_Char_MN_Init(void) 
{
    ...
    LCD_Char_MN_WrCntrlNib(LCD_Char_MN_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(5u);                                                         /* Delay 5 ms */
    LCD_Char_MN_WrCntrlNib(LCD_Char_MN_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(15u);                                                        /* Delay 15 ms */
    LCD_Char_MN_WrCntrlNib(LCD_Char_MN_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(1u);                                                         /* Delay 1 ms */
    LCD_Char_MN_WrCntrlNib(LCD_Char_MN_DISPLAY_4_BIT_INIT);    /* Selects 4-bit mode */
    CyDelay(5u);                                                         /* Delay 5 ms */

    LCD_Char_MN_WriteControl(LCD_Char_MN_CURSOR_AUTO_INCR_ON);    /* Incr Cursor After Writes */
    LCD_Char_MN_WriteControl(LCD_Char_MN_DISPLAY_CURSOR_ON);      /* Turn Display, Cursor ON */
    LCD_Char_MN_WriteControl(LCD_Char_MN_DISPLAY_2_LINES_5x10);   /* 2 Lines by 5x10 Characters */
    LCD_Char_MN_WriteControl(LCD_Char_MN_DISPLAY_CURSOR_OFF);     /* Turn Display, Cursor OFF */
    LCD_Char_MN_WriteControl(LCD_Char_MN_CLEAR_DISPLAY);          /* Clear LCD Screen */
    LCD_Char_MN_WriteControl(LCD_Char_MN_DISPLAY_ON_CURSOR_OFF);  /* Turn Display ON, Cursor OFF */
    LCD_Char_MN_WriteControl(LCD_Char_MN_RESET_CURSOR_POSITION);  /* Set Cursor to 0,0 */
    CyDelay(5u);

    #if(LCD_Char_MN_CUSTOM_CHAR_SET != LCD_Char_MN_NONE)
        LCD_Char_MN_LoadCustomFonts(LCD_Char_MN_customFonts);
    #endif /* LCD_Char_MN_CUSTOM_CHAR_SET != LCD_Char_MN_NONE */
}


/*******************************************************************************
* Function Name: LCD_Char_MN_Enable
********************************************************************************
*
* Summary:
...
*  LCD_Char_MN_Init() turns on the LCD.
*
*******************************************************************************/
void LCD_Char_MN_Enable(void) 
{
    LCD_Char_MN_DisplayOn();
    LCD_Char_MN_enableState = 1u;
}


/*******************************************************************************
* Function Name: LCD_Char_MN_Start
********************************************************************************
*
* Summary:
*  Performs initialization required for the components normal work.
...
*
* Parameters:
*  LCD_Char_MN_initVar - global variable.
*
* Return:
*  LCD_Char_MN_initVar - global variable.
*
...
void LCD_Char_MN_Start(void) 
{
    ...
    if(LCD_Char_MN_initVar == 0u)
    {
        LCD_Char_MN_Init();
        LCD_Char_MN_initVar = 1u;
    }
...
/*******************************************************************************
* Function Name: LCD_Char_MN_Stop
********************************************************************************
*
* Summary:
...
{
    /* Calls LCD Off Macro */
    LCD_Char_MN_DisplayOff();
    LCD_Char_MN_enableState = 0u;
}


/*******************************************************************************
*  Function Name: LCD_Char_MN_Position
********************************************************************************
*
* Summary:
...
void LCD_Char_MN_Position(uint8 row, uint8 column) 
{
    ...
    {
        case (uint8)0:
            LCD_Char_MN_WriteControl(LCD_Char_MN_ROW_0_START + column);
            break;
        case (uint8) 1:
            LCD_Char_MN_WriteControl(LCD_Char_MN_ROW_1_START + column);
            break;
        case (uint8) 2:
            LCD_Char_MN_WriteControl(LCD_Char_MN_ROW_2_START + column);
            break;
        case (uint8) 3:
            LCD_Char_MN_WriteControl(LCD_Char_MN_ROW_3_START + column);
            break;
        default:
...
/*******************************************************************************
* Function Name: LCD_Char_MN_PrintString
********************************************************************************
*
* Summary:
...
void LCD_Char_MN_PrintString(char8 const string[]) 
{
    ...
    {
        LCD_Char_MN_WriteData((uint8)current);
...
/*******************************************************************************
*  Function Name: LCD_Char_MN_PutChar
********************************************************************************
*
* Summary:
...
{
    LCD_Char_MN_WriteData((uint8)character);
}


/*******************************************************************************
*  Function Name: LCD_Char_MN_WriteData
********************************************************************************
*
* Summary:
...
void LCD_Char_MN_WriteData(uint8 dByte) 
{
    ...
    LCD_Char_MN_IsReady();
    nibble = dByte >> LCD_Char_MN_NIBBLE_SHIFT;

    /* Write high nibble */
    LCD_Char_MN_WrDatNib(nibble);

    nibble = dByte & LCD_Char_MN_NIBBLE_MASK;
    /* Write low nibble */
    LCD_Char_MN_WrDatNib(nibble);
}
...
/*******************************************************************************
*  Function Name: LCD_Char_MN_WriteControl
********************************************************************************
*
* Summary:
...
void LCD_Char_MN_WriteControl(uint8 cByte) 
{
    ...
    nibble = cByte >> LCD_Char_MN_NIBBLE_SHIFT;
    
    /* WrCntrlNib(High Nibble) */
    LCD_Char_MN_WrCntrlNib(nibble);
    nibble = cByte & LCD_Char_MN_NIBBLE_MASK;
...
/*******************************************************************************
* Function Name: LCD_Char_MN_IsReady
********************************************************************************
*
* Summary:
...
void LCD_Char_MN_IsReady(void) 
{
    uint8 value;
    uint32 timeout;
    timeout = LCD_Char_MN_READY_DELAY;

    /* Clear LCD port*/
    LCD_Char_MN_PORT_DR_REG &= ((uint8)(~LCD_Char_MN_PORT_MASK));
    ...
    */
        ...
        value = LCD_Char_MN_PORT_PC_REG & ((uint32) (~ LCD_Char_MN_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Char_MN_PORT_PC_REG = value | LCD_Char_MN_HIGH_Z_DATA_DM;
        ...
        /* Mask off data pins to clear old values out */
        value = LCD_Char_MN_PORT_DM0_REG & ((uint8)(~LCD_Char_MN_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Char_MN_PORT_DM0_REG = value | (LCD_Char_MN_HIGH_Z_DM0 & LCD_Char_MN_DATA_MASK);

        /* Mask off data pins to clear old values out */
        value = LCD_Char_MN_PORT_DM1_REG & ((uint8)(~LCD_Char_MN_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Char_MN_PORT_DM1_REG = value;

        /* Mask off data pins to clear old values out */
        value = LCD_Char_MN_PORT_DM2_REG & ((uint8)(~LCD_Char_MN_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Char_MN_PORT_DM2_REG = value;
    
    #endif /* CY_PSOC4 */
    
    /* Make sure RS is low */
    LCD_Char_MN_PORT_DR_REG &= ((uint8)(~LCD_Char_MN_RS));

    /* Set R/W high to read */
    LCD_Char_MN_PORT_DR_REG |= LCD_Char_MN_RW;
    ...
    do
    {
        ...
        /* Set E high */
        LCD_Char_MN_PORT_DR_REG |= LCD_Char_MN_E;

        /* 360 ns delay setup time for data pins */
        CyDelayUs(1u);

        /* Get port state */
        value = LCD_Char_MN_PORT_PS_REG;

        /* Set enable low */
        LCD_Char_MN_PORT_DR_REG &= ((uint8)(~LCD_Char_MN_E));

        /* This gives true delay between disabling Enable bit and polling Ready bit */
        CyDelayUs(0u);

        /* Extract ready bit */
        value &= LCD_Char_MN_READY_BIT;
    ...
    LCD_Char_MN_PORT_DR_REG &= ((uint8)(~LCD_Char_MN_RW));
    ...
    LCD_Char_MN_PORT_DR_REG &= ((uint8)(~LCD_Char_MN_PORT_MASK));
        ...
        value = LCD_Char_MN_PORT_PC_REG & ((uint32)(~ LCD_Char_MN_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        LCD_Char_MN_PORT_PC_REG = value | LCD_Char_MN_STRONG_DATA_DM;
        ...
        */
        LCD_Char_MN_PORT_DM0_REG &= ((uint8)(~LCD_Char_MN_DATA_MASK));
        /* Mask off data pins to clear high z values out */
        value = LCD_Char_MN_PORT_DM1_REG & ((uint8)(~LCD_Char_MN_DATA_MASK));
        /* Configure data pins to Strong Drive, others unchanged */
        LCD_Char_MN_PORT_DM1_REG = value | (LCD_Char_MN_STRONG_DM1 & LCD_Char_MN_DATA_MASK);

        /* Mask off data pins to clear high z values out */
        value = LCD_Char_MN_PORT_DM2_REG & ((uint8)(~LCD_Char_MN_DATA_MASK));
        /* Configure data pins to Strong Drive, others unchanged */
        LCD_Char_MN_PORT_DM2_REG = value | (LCD_Char_MN_STRONG_DM2 & LCD_Char_MN_DATA_MASK);
...
/*******************************************************************************
*  Function Name: LCD_Char_MN_WrDatNib
********************************************************************************
*
* Summary:
...
static void LCD_Char_MN_WrDatNib(uint8 nibble) 
{
    ...
    LCD_Char_MN_PORT_DR_REG |= LCD_Char_MN_RS;
    ...
    /* Clear data pins */
    LCD_Char_MN_PORT_DR_REG &= ((uint8)(~LCD_Char_MN_DATA_MASK));

    /* Write data, bring E high */
    #if(0u != LCD_Char_MN_PORT_SHIFT) /* MISRA forbids shift by 0 so need to handle that */
        LCD_Char_MN_PORT_DR_REG |= 
            (LCD_Char_MN_E | ((uint8)(((uint8) nibble) << LCD_Char_MN_PORT_SHIFT)));
    #else
        LCD_Char_MN_PORT_DR_REG |= (LCD_Char_MN_E | nibble);
    #endif /* (0u != LCD_Char_MN_PORT_SHIFT) */
...
/*******************************************************************************
*  Function Name: LCD_Char_MN_WrCntrlNib
********************************************************************************
*
* Summary:
...
static void LCD_Char_MN_WrCntrlNib(uint8 nibble) 
{
    ...
    LCD_Char_MN_PORT_DR_REG &= ((uint8)(~(LCD_Char_MN_RS | LCD_Char_MN_RW)));
    ...
    /*******************************************************************************
    *  Function Name: LCD_Char_MN_PrintInt8
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        static char8 const CYCODE LCD_Char_MN_hex[16u] = "0123456789ABCDEF";
        
        LCD_Char_MN_PutChar((char8) LCD_Char_MN_hex[value >> LCD_Char_MN_BYTE_UPPER_NIBBLE_SHIFT]);
        LCD_Char_MN_PutChar((char8) LCD_Char_MN_hex[value & LCD_Char_MN_BYTE_LOWER_NIBBLE_MASK]);
    }


    /*******************************************************************************
    *  Function Name: LCD_Char_MN_PrintInt16
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        LCD_Char_MN_PrintInt8((uint8)(value >> LCD_Char_MN_U16_UPPER_BYTE_SHIFT));
        LCD_Char_MN_PrintInt8((uint8)(value & LCD_Char_MN_U16_LOWER_BYTE_MASK));
    }

    
    /*******************************************************************************
    *  Function Name: LCD_Char_MN_PrintInt32
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 shift = LCD_Char_MN_32_BIT_SHIFT;
        ...
        while (shift != 0u)
        {
            ...
            */
            shift -= LCD_Char_MN_8_BIT_SHIFT;
            
            /* Print 8 bits of uint32 hex number */
            LCD_Char_MN_PrintInt8((uint8) ((uint32) (value >> shift)));
        }
    }
    
    
    /*******************************************************************************
    *  Function Name: LCD_Char_MN_PrintNumber
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    *  Function Name: LCD_Char_MN_PrintU32Number
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 tmpDigit;
        char8 number[LCD_Char_MN_NUMBER_OF_REMAINDERS_U32 + 1u];
        uint8 digIndex = LCD_Char_MN_NUMBER_OF_REMAINDERS_U32;
        ...
        */
        ...
        /* Print out number */
        LCD_Char_MN_PrintString(&number[digIndex]);
    }
